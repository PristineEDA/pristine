              ...
              <li key={fileId}>
                <label className="flex cursor-pointer items-center gap-3 rounded-md border border-transparent px-2 py-1.5 text-sm transition-colors hover:border-border/80 hover:bg-background/35">
                  <input
                    ...
                    disabled={isSaving}
                    aria-label={`${getFileBaseName(fileId)} ${fileId}`}
                    onChange={() => toggleFileSelection(fileId)}
                  />
                  <span className="min-w-0 flex-1">
                    <span className="block truncate font-medium">{getFileBaseName(fileId)}</span>
                    <span className="block truncate text-xs text-muted-foreground">{fileId}</span>
                  </span>
                  <span
                    data-testid={`unsaved-changes-file-status-${getFileBaseName(fileId)}`}
        ...
        <DialogFooter>
          ...
          {!isReviewMode && (
            <Button
              ...
              onClick={() => discardUnsavedChanges(selectedRequestedFileIds)}
            >
              {selectedRequestedFileIds.length === requestedFileIds.length ? "Don't Save" : `Don't Save Selected (${selectedRequestedFileIds.length})`}
            </Button>
          )}
          {requestedFileIds.length > 1 && selectedRequestedFileIds.length !== requestedFileIds.length && (
            <Button
              type="button"
              variant="outline"
          ...
          <Button
            ...
            disabled={isSaving || selectedRequestedFileIds.length === 0}
            onClick={() => {
              void confirmUnsavedChangesSave(selectedRequestedFileIds);
            }}
          >
            {isSaving
              ? 'Saving...'
