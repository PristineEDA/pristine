...
export function OutlineIcon({ type }: { type: OutlineItem['type'] }) {
  switch (type) {
    ...
    case 'input': return <span className="text-ide-success text-[10px] font-bold font-mono">I</span>;
    case 'output': return <span className="text-ide-syntax-function text-[10px] font-bold font-mono">O</span>;
    case 'inout': return <span className="text-ide-info text-[10px] font-bold font-mono">IO</span>;
    case 'wire': return <span className="text-ide-info text-[10px] font-bold">W</span>;
    case 'reg': return <span className="text-ide-error text-[10px] font-bold">R</span>;
    ...
    case 'function': return <Code2 size={12} className="text-ide-success" />;
    ...
    default: return <Circle size={10} className="text-ide-text-muted" />;
  }
}
...
export const OutlineNode = memo(function OutlineNode({
  ...
  return (
    <div>
      <div
        className="flex items-center gap-1 h-6 cursor-pointer hover:bg-ide-hover transition-colors text-ide-text"
      ...
      >
        ...
        {item.detail && (
          <span className="text-[11px] text-ide-text-muted pr-2 truncate max-w-20">{item.detail}</span>
        )}
        <span className="text-[10px] text-ide-text-muted pr-2 opacity-0 hover:opacity-100"
